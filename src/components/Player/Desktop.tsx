...
const DesktopPlayer: FC<PlayerProps> = ({ playerKey, sources, subtitles }) => {
  ...
  const [playbackSpeed, setPlaybackSpeed] = useState(
    Number(localStorage.getItem("WatchFilms-speed")) || 1
  );
  ...
  const [volume, setVolume] = useState(
    isNaN(parseInt(localStorage.getItem("WatchFilms-volume") as string))
      ? 100
      : Number(localStorage.getItem("WatchFilms-volume"))
  );
  ...
  useEffect(() => {
    ...
    localStorage.setItem("WatchFilms-volume", String(volume));
  }, [volume]);
  ...
  useEffect(() => {
    ...
    localStorage.setItem("WatchFilms-speed", String(playbackSpeed));
  ...
  return (
    <div className="relative w-full h-0 pb-[56.25%]">
      <div
        ...
        {paused && (
          <div
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 cursor-pointer transition-all duration-300"
            onClickCapture={handleScreenClicked}>
          ...
          <div
            ...
            <div className="h-[4px] bg-[#FFFFFF80]">
              <div
                ...
                className="h-full bg-white relative after:absolute after:w-[12px] after:h-[12px] after:right-[-6px] after:bg-white after:top-1/2 after:-translate-y-1/2 after:rounded-full after:opacity-0 seek-ball after:transition"></div>
            </div>
          </div>
          <div className="flex justify-between items-stretch flex-grow px-4">
            <div className="flex items-center gap-4">
              <button
                ...
                {paused ? (
                  <i className="fa-solid fa-play h-[20px] w-[20px]"></i>
                ) : (
                  <i className="fa-solid fa-pause h-[20px] w-[20px]"></i>
                )}
              </button>
